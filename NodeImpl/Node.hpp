...
struct NodeAccessor
{
    ...
    {   
        try
        {
            auto visitor = std::any_cast<ConcreteVisitor<VisitedNodeT>*>(_visitor);
            (*visitor)(t);
        }
    ...
    NodeAccessor(VisitorT& accesor):
       _visitor(std::make_any<VisitorT*>(&accesor))
    {}
private:
    std::any _visitor;
};
...
public:
    ...
    Node(NodeType s): 
        ptrToNode_(std::make_unique<NodeOwner<NodeType>>(std::move(s))) {}
    ...
    struct NodeOwner final: 
        ...
        NodeOwner(NodeType r) :
            node_(std::move(r)) {
        }

        NodeOwner(NodeOwner<NodeType>& r) :
            node_(std::forward<NodeType>(r.node_)) {
        }

        std::unique_ptr<NodeBase> copy_() override {
            return std::make_unique<NodeOwner<NodeType>>(*this);
        }
